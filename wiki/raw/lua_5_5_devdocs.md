# Lua 5.5 Official Documentation
...
The integer keys in the registry are used by the reference mechanism (see [`luaL_ref`](#luaL_ref)), with some predefined values. Therefore, integer keys in the registry must not be used for other purposes.
...
* basic library ([§6.2](#6.2));
* coroutine library ([§6.3](#6.3));
* package library ([§6.4](#6.4));
* string manipulation ([§6.5](#6.5));
* basic UTF-8 support ([§6.6](#6.6));
* table manipulation ([§6.7](#6.7));
* mathematical functions ([§6.8](#6.8)) (sin, log, etc.);
* input and output ([§6.9](#6.9));
* operating system facilities ([§6.10](#6.10));
* debug facilities ([§6.11](#6.11)).

Except for the basic and the package libraries, each library provides all its functions as fields of a global table or as methods of its objects.

## 6.1 – Loading the Libraries in C code

A C host program must explicitly load the standard libraries into a state, if it wants its scripts to use them. For that, the host program can call the function [`luaL_openlibs`](#luaL_openlibs). Alternatively, the host can select which libraries to open, by using [`luaL_openselectedlibs`](#luaL_openselectedlibs). Both functions are declared in the header file `lualib.h`.

The stand-alone interpreter `lua` (see [§7](#7)) already opens all standard libraries.
...
Returns an iterator function that, each time it is called, returns the next captures from `pattern` (see [§6.5.1](#6.5.1)) over the string `s`. If `pattern` specifies no captures, then the whole match is produced in each call. A third, optional numeric argument `init` specifies where to start the search; its default value is 1 and can be negative.
...
Returns a copy of `s` in which all (or the first `n`, if given) occurrences of the `pattern` (see [§6.5.1](#6.5.1)) have been replaced by a replacement string specified by `repl`, which can be a string, a table, or a function. `gsub` also returns, as its second value, the total number of matches that occurred. The name `gsub` comes from *Global SUBstitution*.
...
Returns a binary string containing the values `v1`, `v2`, etc. serialized in binary form (packed) according to the format string `fmt` (see [§6.5.2](#6.5.2)).

### `string.packsize (fmt)`

Returns the length of a string resulting from [`string.pack`](#pdf-string.pack) with the given format. The format string cannot have the variable-length options '`s`' or '`z`' (see [§6.5.2](#6.5.2)).
